...
ssh -i /home/tomcat/.ssh/autobuild_key autobuild@yad124.tt.wgenhq.net "cd $remote_webdriver_workspace && export ENV_PROPERTY_PREFIX=$ENV_PROPERTY_PREFIX && export RUN_ONLY_SMOKE=true && Xvfb :5 -screen 0 1024x768x24 >/dev/null 2>&1 & export DISPLAY=:5.0 && rm -rf tmp && mkdir tmp && echo `pwd` && unzip webdriverbundle.zip -d tmp && cd tmp && ant test-webdriver-precompiled"
