...
if [ $isnightlybuild != 'true' ] && [ "$nextrpmrepo" != "" ]; then
    ...
    cp $buildrpmrepo/mclass-tt-$app-$rpmversion-*-integration.noarch.rpm $nextrpmrepo/mclass-tt-$app-$rpmversion-$buildnumber.noarch.rpm
    cp $buildrpmrepo/tt-migrations-$migrationsappname-$rpmversion-*-integration.noarch.rpm $nextrpmrepo/tt-migrations-$migrationsappname-$rpmversion-$buildnumber.noarch.rpm

    if [ "$othermigrationsappname" != "" ]
    then
        cp $buildrpmrepo/tt-migrations-$othermigrationsappname-$rpmversion-*-integration.noarch.rpm $nextrpmrepo/tt-migrations-$othermigrationsappname-$rpmversion-$buildnumber.noarch.rpm
    fi
