...
# time ./conf/base/scripts/build/run_parallel_tests.sh yad127.tt.wgenhq.net,yad128.tt.wgenhq.net,yad129.tt.wgenhq.net autobuild ~/.ssh/id_rsa /home/autobuild/dan-outcomes OUTCOMES_HOME

# example ci usage (get $JOB_NAME from hudson) for outcomes:
# time ./conf/base/scripts/build/run_parallel_tests.sh yad127.tt.wgenhq.net,yad128.tt.wgenhq.net,yad129.tt.wgenhq.net autobuild /home/tomcat/.ssh/autobuild_key /home/autobuild/$JOB_NAME OUTCOMES_HOME
...
find target/test/integration target/test/webservice \
  ...
  | python conf/base/scripts/build/parallelTests.py -s $slaves -u $masteruser -i $identityfile -w $slaveworkspace -v $apphomeenvvar
