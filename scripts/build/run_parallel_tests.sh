...
# Finds all integration and webservice tests and runs them on $slaves as $sshuser in an automatically-created $slaveworkspace remote directory.
...
# example ci usage (gets $JOB_NAME from hudson) for outcomes:
...
testsperbatch=$6  # e.g., 8, to farm 8 tests to each slave at a time

find target/test/integration target/test/webservice \
  | grep Test.class \
  | xargs -i basename {} .class \
  | python conf/base/scripts/build/parallelTests.py -s $slaves -u $sshuser -i $identityfile -w $slaveworkspace -v $apphomeenvvar -n $testsperbatch
